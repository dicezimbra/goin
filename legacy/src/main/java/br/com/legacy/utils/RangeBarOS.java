...
//    Range Bar modified from com.appyvet.rangebar.Bar

public class RangeBarOS extends View {
    ...
    private PinViewOS mLeftThumb;

    private PinViewOS mRightThumb;

    private BarOS mBar;

    private ConnectingLineOS mConnectingLine;

    private RangeBarOS.OnRangeBarChangeListener mListener;

    private RangeBarOS.OnRangeBarTextListener mPinTextListener;
    ...
    private RangeBarOS.PinTextFormatter mPinTextFormatter = new RangeBarOS.PinTextFormatter() {
        @Override
    ...
    public RangeBarOS(Context context) {
        super(context);
    }

    public RangeBarOS(Context context, AttributeSet attrs) {
        super(context, attrs);
        rangeBarInit(context, attrs);
    }

    public RangeBarOS(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mBar = new BarOS(ctx, marginLeft, yPos, barLength, mTickCount, mTickHeightDP, mTickColor,
        ...
        mConnectingLine = new ConnectingLineOS(ctx, yPos, mConnectingLineWeight,
     ...
     *                 existing listener
     */
    public void setOnRangeBarChangeListener(RangeBarOS.OnRangeBarChangeListener listener) {
        mListener = listener;
    }
     ...
     *                         existing listener
     */
    public void setPinTextListener(RangeBarOS.OnRangeBarTextListener mPinTextListener) {
    ...
    public void setPinTextFormatter(RangeBarOS.PinTextFormatter pinTextFormatter) {
    ...
    private void createConnectingLine() {
        ...
        mConnectingLine = new ConnectingLineOS(getContext(),
                getYPos(),
    ...
    private void onActionDown(float x, float y) {
        if (mIsRangeBar) {
            if(mLeftThumb.isInTargetZone(x,y) && mRightThumb.isInTargetZone(x,y)){
                float diffLeft = Math.abs(mLeftThumb.getX() - x);
                float diffTight = Math.abs(mRightThumb.getX() - x);
                if (diffLeft < diffTight){
                    pressPin(mLeftThumb);
                } else {
    ...
    private void onActionMove(float x) {
        ...
        if (mIsRangeBar && mLeftThumb.isPressed()) {
            oldX = mLeftThumb.getX();
            if(x >= mRightThumb.getX()){
        ...
        if (mIsRangeBar && mLeftThumb.getX() > mRightThumb.getX()) {
            final PinViewOS temp = mLeftThumb;
        ...
        if(newLeftIndex == newRightIndex){
            if(mLeftThumb.isPressed()){
                newLeftIndex -= 1;
                movePin(mLeftThumb,oldX);

            } else {
                newRightIndex += 1;
                movePin(mRightThumb,oldX);
        ...
        if (x <= componentLeft && mLeftThumb.isPressed()) {
            newLeftIndex = 0;
            movePin(mLeftThumb, mBar.getLeftX());
        } else if (x >= componentRight && mRightThumb.isPressed()) {
     ...
     */
    private void pressPin(final PinViewOS thumb) {
        if (mFirstSetTickCount) {
            mFirstSetTickCount = false;
        }
     ...
     */
    private void releasePin(final PinViewOS thumb) {
     ...
     */
    private void movePin(PinViewOS thumb, float x) {
    ...
    public interface OnRangeBarChangeListener {
        ...
        void onRangeChangeListener(RangeBarOS rangeBar, int leftPinIndex,
    ...
    public interface OnRangeBarTextListener {
        ...
        String getPinValue(RangeBarOS rangeBar, int tickIndex);
    }
}
